...
print("\t (1) Consola \t (2) Txt")
a = 1
while a != 0: #Se valida que la opción sea válida
  lectura = int(input("L: ")) #Se determina el tipo de lectura de entrada, si es 1: se ingresarán los datos manualmente y 
  #si es 2: se ingresa por medio de un txt siguiente las instrucciones dados. Veáse el ejemplo comentado.
  if lectura > 0 and lectura < 3:
    a = 0
  else: 
...
if lectura == 1:
  print("Cuantas variables considera la función de optimizacion?")
  nZ = int(input())
  print("Cuantas restricciones considerara?")
  nR = int(input())
...
if lectura == 1:
  #Solicitando restricciones a considerar
  for i in range(filas):
    for j in range(colum):
      if j == 0 and i != filas - 1: #se llena la primera columna
        tabla[i,j] = 0 
      elif j == 0 and i == filas - 1:
        tabla[i,j] = 1 #Para el valor inicial de Z
        
      elif 0 < j <= nZ and i != filas - 1:
        tabla[i,j] = float(input(f"x{j} de la restriccion {i+1}: ")) #Se almacenan las restricciones
        tablaD[i,j-1] = float(tabla[i,j])
      elif j == colum - 1 and i != filas - 1:
        tabla[i,j] = float(input("Es igualada a: "))
        tablaD[i,-1] = float(tabla[i,j])
      elif j == colum - 1 and i == filas - 1:
        tabla[i,j] = 0
      elif  nZ < j < colum-1:
        if i== j - nZ-1:
          b= 1
          while b != 0:
            print(f"Signo de la restriccion. (1) >=   (2) =     (3) <=")
            signo = int(input())
            if signo == 1:
              tabla[i,j] = -1 # se identifica el signo de la restriccion
            if signo == 2:
              tabla[i,j] = 1
            if signo == 3:
              tabla[i,j] = 1
            if signo > 0 and signo < 4:
              b = 0
            else:
              print("Signo incorrecto :(") #Si el input es diferente a los parámetros indicado, se vuelve a pedir hasta que sea válido.
          tablaS[i,0] = float(tabla[i,j])
        else:
          tabla[i,j] = 0

# se solicita la información de las variables de la función de optimización
if lectura == 1:
  print("Optimizando por medio del método BigM: ")
  for i in range(filas):
    for j in range(colum):
      if 0 < j <= nZ and i == filas - 1: #solicitando datos
        tabla[i,j] =float(input(f"x{j} de la funcion Z: "))
        tablaD[i,j-1] =float(tabla[i,j])
        #if tabla[i, j] >= 0: 
...
tablaDD[nRd,0] = 1
print("Resultado problema primal: ")
print(tablaDD)
...
def pivote(matriz, colum, filas): #se encuentra la fila del pivote y por lo tanto, el pivote
    global fila_pivot
...
def evaluando(tablainv,tabla, colum): #fila que se evaluara con el pivote
    for j in range(colum):
...
#tabla = add_artif(filas,tabla,enc)
#colum = len(tabla[0,:])
...
tabla = add_artif(filas, tabla)
enc = add_art_enc(filas, enc)
colum = len(tabla[0,:])
tablainv = np.zeros((filas,colum))
print(enc)
...
solucion(decision, colum, filas, tabla, enc)
...
tablaDD = add_artif(filasD, tablaDD)
enc = add_art_enc(filasD, enc)
...
while continuarD == 1:
    ...
    epivote = pivote(tablaDD, columD, filasD)
    print(colum_pivote)
    print(epivote) #Para poder visualizar el elemento pivote encontrado
    evaluando(tablainvD, tablaDD, columD)
...
print("Problema dual: ")
solucion(decision, columD, filasD, tablaDD, enc)